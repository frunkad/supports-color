...
const translateLevel = (level: number): false | { level: number; hasBasic: boolean; has256: boolean; has16m: boolean; } => {
	if (level === 0) {
		return false;
	}
...
const checkSupportsColor = (haveStream: Deno.Writer & Deno.Closer & { rid: number }, streamIsTTY: boolean): number => {
    const { args, env } = Deno;
    ...
    if (Deno.env.get('FORCE_COLOR') !== undefined) {
        if (Deno.env.get('FORCE_COLOR') === "true") {
            ...
            forceColor = Deno.env.get('FORCE_COLOR')!.length === 0 ? 1 : Math.min(parseInt(Deno.env.get('FORCE_COLOR')!, 10), 3);
        }
    }
	...
	if (env.get('CI') !== undefined) {
		if (['TRAVIS', 'CIRCLECI', 'APPVEYOR', 'GITLAB_CI', 'GITHUB_ACTIONS'].some(sign => sign in env.toObject()) || env.get('CI_NAME') === 'codeship') {
			return 1;
		}
...
const getSupportLevel = (stream: Deno.Writer & Deno.Closer & { rid: number }) => {
