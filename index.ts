...
const translateLevel = (level: number): false | ColorCheckResult => {
  if (level === 0) {
    return false;
  }
...
const checkSupportsColor = (
  haveStream: Deno.Writer & Deno.Closer & { rid: number },
  streamIsTTY: boolean,
): number => {
  const { args, env } = Deno;
  ...
  if (
    ...
    parsedArgs.color !== undefined &&
      (parsedArgs.color === false || parsedArgs.color === "false" ||
        parsedArgs.color === "never")
  ) {
    forceColor = 0;
  } else if (
    ...
    parsedArgs.color !== undefined &&
      (parsedArgs.color === true || parsedArgs.color === "true" ||
        parsedArgs.color === "always")
  ) {
    forceColor = 1;
  }
  ...
  if (Deno.env.get("FORCE_COLOR") !== undefined) {
    if (Deno.env.get("FORCE_COLOR") === "true") {
      ...
      forceColor = Deno.env.get("FORCE_COLOR")!.length === 0
        ? 1
        : Math.min(parseInt(Deno.env.get("FORCE_COLOR")!, 10), 3);
    }
  }
  ...
  if (
    parsedArgs.color !== undefined &&
    (parsedArgs.color == "16m" || (parsedArgs.color == "full") ||
      (parsedArgs.color == "truecolor"))
  ) {
    return 3;
  }
  ...
  if (env.get("CI") !== undefined) {
    if (
      ["TRAVIS", "CIRCLECI", "APPVEYOR", "GITLAB_CI", "GITHUB_ACTIONS"].some(
        (sign) => sign in env.toObject()
  ...
  if (env.get("TEAMCITY_VERSION") !== undefined) {
    return /^(9\.(0*[1-9]\d*)\.|\d{2,}\.)/.test(env.get("TEAMCITY_VERSION")!)
      ? 1
      : 0;
  }
...
export const supportColorCheck = () => {
  return {
    supportsColor: getSupportLevel,
    stdout: translateLevel(
      checkSupportsColor(Deno.stdout, Deno.isatty(Deno.stdout.rid)),
    ),
    stderr: translateLevel(
      checkSupportsColor(Deno.stderr, Deno.isatty(Deno.stderr.rid)),
    ),
  };
};
